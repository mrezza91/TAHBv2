package ta.bismillahirahmanirohim.muhammadrezza.tahb;

import android.app.Application;
import android.content.Intent;
import android.support.v7.appcompat.*;
import android.support.v7.app.AppCompatActivity;
import android.os.Bundle;
import android.support.v7.widget.Toolbar;
import android.view.View;
import android.widget.Button;

public class healthinfo extends AppCompatActivity {

    private Button btnhisto;
    private Toolbar toolbar1;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_healthinfo);


        btnhisto=(Button) findViewById(R.id.btnhisto);
        toolbar1= (Toolbar) findViewById(R.id.app_bar2);

        setSupportActionBar(toolbar1);
        getSupportActionBar().setTitle("My Health Info");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);

        btnhisto.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent= new Intent(healthinfo.this,histhb.class);
                startActivity(intent);
                finish();
            }
        });



        toolbar1.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent= new Intent(healthinfo.this,MainActivity.class);
                startActivity(intent);
                finish();
            }
        });
    }
}
